...
public class SpellcheckableMessageArea extends Composite {
	...
	//
	// public TextEditorPropertyAction(String label, SourceViewer viewer, String
	// preferenceKey) {
	...
	public SpellcheckableMessageArea(Composite parent, String initialText,
		...
		configuration = new HyperlinkSourceViewer.Configuration(
			...
			public IContentAssistant getContentAssistant(ISourceViewer viewer) {
				...
				// createContentAssistActionHandler(sourceViewer);
				return assistant;
			}
	...
	private void configureHardWrap() {
		if (shouldHardWrap()) {
			...
			// textWidget.setText(textWidget.getText()); // XXX: workaround for
			// https://bugs.eclipse.org/384886
	...
	//
	// final TextEditorPropertyAction showWhitespaceAction = new
	...
	// final SourceViewerDecorationSupport support = new
